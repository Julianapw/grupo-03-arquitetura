...
|---|---|---|---|---|
| **Monolito em camadas** (cap. 5) | **Em parte.** | Subdomínio de Atendimento: emissão de segunda via, contestação e cadastro de gratuidade. | Serviria em parte, aplicando-se ao subdomínio de Atendimento (emissão de segunda via, contestação e cadastro de gratuidade). Esse estilo é adequado para subdomínios com comportamento predominante de cadastro e consulta, com regras concentradas e sem variação por cliente (Seção 5.5). Entretanto, não é adequado para os demais subdomínios do envelope B, pois como tem baixa implantabilidade, baixa escalabilidade e baixa disponibilidade (Apêndice A), não atende bem sistemas de alto volume de funcionalidades e regras diferentes mantidos por grandes equipes com ciclos e momentos de implantação independentes (Seção 5.6). Por utilizar uma única unidade de implantação (Seção 5.1), uma falha no deploy de um módulo derrubaria sistema inteiro, gerando multa contratual de SLA por indisponibilidade. (**Livro: Estilos Arquiteturais de Software: guia de consulta**) | **Melhora:** Testabilidade (facilita testar as regras de negócio isoladas) e Simplicidade inicial (operação simples para fluxos de cadastro/consulta). **Piora:** Disponibilidade (falha em um módulo cai o sistema todo e gera multa de SLA), Implantabilidade e Escalabilidade (40 devs em 5 times geram travamentos e conflitos de código). |
...
| **SOA e barramento de serviços (ESB)** (cap. 10) | **Em parte.** | Integração externa; repasse e conciliação. | O ESB é adequado para integrar sistemas heterogêneos pertencentes a diferentes empresas, centralizando tradução de dados, auditoria e contratos. No consórcio, pode facilitar a comunicação com bancos, adquirentes, operadoras e prefeitura, mas deve ser evitado na validação da passagem, pois acrescenta um salto de comunicação e pode se tornar um ponto único de falha (**Livro, §§ 10.5-10.7**). | **Melhora:** interoperabilidade, ao centralizar a integração entre sistemas heterogêneos. **Piora:** disponibilidade, pois o barramento pode se tornar um ponto único de falha e precisa de alta disponibilidade. |
...
| **Serverless** (cap. 12) | **Em parte.** | Subdomínio de Informação ao Passageiro (previsão de chegada, aplicativo, painéis) e Integração Externa (órgão gestor, operadoras, banco, adquirente). | Serviria em parte, aplicando-se aos subdomínios de Informação ao Passageiro (notificações e alertas) e Integração Externa (webhooks e tarefas agendadas). Esse estilo é ideal para tarefas curtas acionadas por eventos e com fluxo irregular, pois evita gastos com servidores parados sem uso (Seção 12.5). Por outro lado, é inadequado para a Telemetria da Frota no Envelope B, já que a cobrança por invocação em cargas gigantes e contínuas (400 sinais/s) sai muito mais cara do que uma estrutura dedicada (Seção 12.6). Além disso, não atende a Validação Embarcada porque o atraso da partida a frio (cold start) ao criar novas instâncias compromete o tempo de resposta exigido pela catraca (Seção 12.2 e Seção 12.6). (**Livro: Estilos Arquiteturais de Software: guia de consulta**) | **Melhora:** Custabilidade / Eficiência Financeira (elimina gastos com servidores ociosos em tarefas eventuais) e Escalabilidade Automática (escala instantaneamente sob demanda nos picos). **Piora:** Performance / Latência (atrasos causados pelo cold start) e Custabilidade em Carga Contínua (custo elevado para processamento ininterrupto de telemetria). |
| **Arquitetura celular (cell-based)** (cap. 13) | **Sim.** | Cartões e recarga; backend da validação. | A arquitetura celular permite dividir o sistema em células independentes, limitando o impacto de uma falha a apenas uma parcela da base. Isso é especialmente adequado ao cenário do grupo, pois a disponibilidade possui valor contratual, com SLA de 99,9% e multa por indisponibilidade da validação e da recarga (**Livro, §§ 13.5-13.7**). | **Melhora:** disponibilidade, pois reduz o raio de impacto das falhas. **Piora:** custo, devido à necessidade de manter, observar e atualizar múltiplas instalações e replicar componentes de infraestrutura. |
